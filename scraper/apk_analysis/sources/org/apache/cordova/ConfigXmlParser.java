...
public class ConfigXmlParser {
    ...
    public void parse(Context action) {
        int identifier = action.getResources().getIdentifier(FirebaseConstants.Commands.CONFIGURE, "xml", action.getClass().getPackage().getName());
        if (identifier == 0 && (identifier = action.getResources().getIdentifier(FirebaseConstants.Commands.CONFIGURE, "xml", action.getPackageName())) == 0) {
    ...
    public void handleStartTag(XmlPullParser xml) {
        ...
        if (this.insideFeature && name.equals("param")) {
            ...
            if (this.paramType.equals(SentryStackFrame.JsonKeys.PACKAGE) || this.paramType.equals("android-package")) {
    ...
    private String getLaunchUrlPrefix() {
        ...
        if (!lowerCase.contentEquals(SCHEME_HTTP) && !lowerCase.contentEquals(SCHEME_HTTPS)) {
            LOG.d(TAG, "The provided scheme \"" + lowerCase + "\" is not valid. Defaulting to \"" + SCHEME_HTTPS + "\". (Valid Options=" + SCHEME_HTTP + FirebaseConstants.SEPARATOR + SCHEME_HTTPS + ")");
            lowerCase = SCHEME_HTTPS;
        }
        return lowerCase + "://" + string + JsonPointer.SEPARATOR;
    }
