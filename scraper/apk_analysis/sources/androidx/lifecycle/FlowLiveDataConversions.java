...
public final class FlowLiveDataConversions {
    ...
    public static final <T> LiveData<T> asLiveData(Flow<? extends T> flow, CoroutineContext context, long j) {
        Intrinsics.checkNotNullParameter(flow, "<this>");
        Intrinsics.checkNotNullParameter(context, "context");
        Xa xa = (LiveData<T>) CoroutineLiveDataKt.liveData(context, j, new FlowLiveDataConversions$asLiveData$1(flow, null));
        if (flow instanceof StateFlow) {
            if (ArchTaskExecutor.getInstance().isMainThread()) {
                xa.setValue(((StateFlow) flow).getValue());
            } else {
                xa.postValue(((StateFlow) flow).getValue());
            }
        }
        return xa;
    }
