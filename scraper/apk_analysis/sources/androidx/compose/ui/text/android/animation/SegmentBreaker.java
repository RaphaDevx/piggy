...
public final class SegmentBreaker {
    ...
    private final List<Segment> breakSegmentWithWord(LayoutHelper layoutHelper, boolean dropSpaces) {
        int i;
        Layout layout = layoutHelper.getLayout();
        int ceil = (int) Math.ceil(layout.getPaint().measureText(StringUtils.SPACE));
        ...
        if (breakOffsets.size() != 0) {
            ...
            if (breakOffsets.size() != 1) {
                ...
                while (true) {
                    ...
                    boolean z2 = layout.getParagraphDirection(lineForOffset) == -1 ? r5 : z;
                    boolean isRtlCharAt = layout.isRtlCharAt(intValue2);
                    int ceil2 = (int) Math.ceil(layoutHelper.getHorizontalPosition(intValue2, isRtlCharAt == z2 ? r5 : z, z));
                    int ceil3 = (int) Math.ceil(layoutHelper.getHorizontalPosition(intValue, isRtlCharAt == z2 ? r5 : false, r5));
