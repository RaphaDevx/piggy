    ...
    public boolean isForceStopped() {
        ...
        try {
            ...
            if (Build.VERSION.SDK_INT >= 30) {
                ...
                historicalProcessExitReasons = ((ActivityManager) this.mContext.getSystemService(DataSources.EventTypeValue.ACTIVITY_EVENT_TYPE)).getHistoricalProcessExitReasons(null, 0, 0);
                if (historicalProcessExitReasons != null && !historicalProcessExitReasons.isEmpty()) {
                    long lastForceStopEventMillis = this.mPreferenceUtils.getLastForceStopEventMillis();
                    for (int i = 0; i < historicalProcessExitReasons.size(); i++) {
                        ApplicationExitInfo m = N9$$ExternalSyntheticApiModelOutline0.m(historicalProcessExitReasons.get(i));
                        reason = m.getReason();
                        if (reason == 10) {
