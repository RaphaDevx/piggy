...
public final class TextFieldPreparedSelection extends BaseTextPreparedSelection<TextFieldPreparedSelection> {
    ...
    public TextFieldPreparedSelection(TextFieldValue currentValue, OffsetMapping offsetMapping, TextLayoutResultProxy textLayoutResultProxy, TextPreparedSelectionState state) {
        super(currentValue.getAnnotatedString(), currentValue.getSelection(), textLayoutResultProxy == null ? null : textLayoutResultProxy.getValue(), offsetMapping, state, null);
    ...
    public final TextFieldValue getValue() {
        return TextFieldValue.m2872copy3r_uNRQ$default(this.currentValue, getAnnotatedString(), getSelection(), (TextRange) null, 4, (Object) null);
    }

    public final TextFieldPreparedSelection moveCursorUpByPage() {
        TextLayoutResultProxy layoutResultProxy;
        TextFieldPreparedSelection textFieldPreparedSelection = this;
        if (textFieldPreparedSelection.getText$foundation_release().length() > 0 && (layoutResultProxy = getLayoutResultProxy()) != null) {
    ...
    private final int jumpByPagesOffset(TextLayoutResultProxy textLayoutResultProxy, int i) {
        ...
        Rect cursorRect = textLayoutResultProxy.getValue().getCursorRect(getOffsetMapping().originalToTransformed(TextRange.m2761getEndimpl(this.currentValue.getSelection())));
        return getOffsetMapping().transformedToOriginal(textLayoutResultProxy.getValue().m2749getOffsetForPositionk4lQ0M(OffsetKt.Offset(cursorRect.getLeft(), cursorRect.getTop() + (Size.m1091getHeightimpl(rect.m1058getSizeNHjbRc()) * i))));
    }

    public final TextFieldPreparedSelection deleteIfSelectedOr(Function1<? super TextFieldPreparedSelection, Unit> or) {
        Intrinsics.checkNotNullParameter(or, "or");
        TextFieldPreparedSelection textFieldPreparedSelection = this;
        textFieldPreparedSelection.getState().resetCachedX();
        if (textFieldPreparedSelection.getText$foundation_release().length() > 0) {
