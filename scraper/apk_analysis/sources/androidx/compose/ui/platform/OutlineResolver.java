...
public final class OutlineResolver {
    ...
    public OutlineResolver(Density density) {
        ...
        this.size = Size.INSTANCE.m1103getZeroNHjbRc();
    ...
    public final boolean m2649isInOutlinek4lQ0M(long position) {
        androidx.compose.ui.graphics.Outline outline;
        if (this.outlineNeeded && (outline = this.calculatedOutline) != null) {
            return ShapeContainingUtilKt.isInOutline(outline, Offset.m1025getXimpl(position), Offset.m1026getYimpl(position), this.tmpTouchPointPath, this.tmpOpPath);
        }
        return true;
    }

    /* renamed from: update-uvyYCjk, reason: not valid java name */
    public final void m2650updateuvyYCjk(long size) {
        if (Size.m1090equalsimpl0(this.size, size)) {
            return;
        }
        this.size = size;
        this.cacheIsDirty = true;
    ...
    private final void updateCache() {
        if (this.cacheIsDirty) {
            ...
            if (this.outlineNeeded && Size.m1094getWidthimpl(this.size) > 0.0f && Size.m1091getHeightimpl(this.size) > 0.0f) {
                this.isSupportedOutline = true;
                androidx.compose.ui.graphics.Outline mo181createOutlinePq9zytI = this.shape.mo181createOutlinePq9zytI(this.size, this.layoutDirection, this.density);
                this.calculatedOutline = mo181createOutlinePq9zytI;
                if (mo181createOutlinePq9zytI instanceof Outline.Rectangle) {
                    updateCacheWithRect(((Outline.Rectangle) mo181createOutlinePq9zytI).getRect());
                    return;
                } else if (mo181createOutlinePq9zytI instanceof Outline.Rounded) {
                    updateCacheWithRoundRect(((Outline.Rounded) mo181createOutlinePq9zytI).getRoundRect());
                    return;
                } else {
                    if (mo181createOutlinePq9zytI instanceof Outline.Generic) {
                        updateCacheWithPath(((Outline.Generic) mo181createOutlinePq9zytI).getPath());
                        return;
                    }
                    return;
                }
    ...
    private final void updateCacheWithRoundRect(RoundRect roundRect) {
        float m1000getXimpl = CornerRadius.m1000getXimpl(roundRect.m1075getTopLeftCornerRadiuskKHJgLs());
        if (RoundRectKt.isSimple(roundRect)) {
            this.cachedOutline.setRoundRect(MathKt.roundToInt(roundRect.getLeft()), MathKt.roundToInt(roundRect.getTop()), MathKt.roundToInt(roundRect.getRight()), MathKt.roundToInt(roundRect.getBottom()), m1000getXimpl);
            return;
        }
