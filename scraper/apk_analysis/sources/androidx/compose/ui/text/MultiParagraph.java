...
public final class MultiParagraph {
    ...
    */
    ...
    public static /* synthetic */ void m2689paintRPmYEkk$default(MultiParagraph multiParagraph, Canvas canvas, long j, Shadow shadow, TextDecoration textDecoration, int i, Object obj) {
        if ((i & 2) != 0) {
            j = Color.INSTANCE.m1290getUnspecified0d7_KjU();
        }
        multiParagraph.m2692paintRPmYEkk(canvas, j, (i & 4) != 0 ? null : shadow, (i & 8) != 0 ? null : textDecoration);
    }

    /* renamed from: paint-RPmYEkk, reason: not valid java name */
    public final void m2692paintRPmYEkk(Canvas canvas, long color, Shadow shadow, TextDecoration decoration) {
        ...
        if (size >= 0) {
            ...
            while (true) {
                ...
                paragraphInfo.getParagraph().mo2695paintRPmYEkk(canvas, color, shadow, decoration);
    ...
    public final Path getPathForRange(int start, int end) {
        ...
        if (size3 >= 0) {
            while (true) {
                ...
                Path.DefaultImpls.m1457addPathUv8p0NA$default(Path, paragraphInfo2.toGlobal(paragraphInfo2.getParagraph().getPathForRange(paragraphInfo2.toLocalIndex(start), paragraphInfo2.toLocalIndex(end))), 0L, 2, null);
                if (i6 > size3) {
                    break;
                }
                i = i6;
    ...
    public final int m2690getOffsetForPositionk4lQ0M(long position) {
        int lastIndex;
        if (Offset.m1026getYimpl(position) <= 0.0f) {
            lastIndex = 0;
        } else {
            lastIndex = Offset.m1026getYimpl(position) >= this.height ? CollectionsKt.getLastIndex(this.paragraphInfoList) : MultiParagraphKt.findParagraphByY(this.paragraphInfoList, Offset.m1026getYimpl(position));
        }
        ParagraphInfo paragraphInfo = this.paragraphInfoList.get(lastIndex);
        if (paragraphInfo.getLength() == 0) {
            return Math.max(0, paragraphInfo.getStartIndex() - 1);
        }
        return paragraphInfo.toGlobalIndex(paragraphInfo.getParagraph().mo2693getOffsetForPositionk4lQ0M(paragraphInfo.m2698toLocalMKHz9U(position)));
    }
    ...
    public final long m2691getWordBoundaryjx7JFs(int offset) {
        requireIndexInRange(offset);
        ParagraphInfo paragraphInfo = this.paragraphInfoList.get(MultiParagraphKt.findParagraphByIndex(this.paragraphInfoList, offset));
        return paragraphInfo.m2697toGlobalGEjPoXI(paragraphInfo.getParagraph().mo2694getWordBoundaryjx7JFs(paragraphInfo.toLocalIndex(offset)));
    }
