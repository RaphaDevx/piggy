...
public final class MapEditorUtils {
    ...
    public final String ellipsizeTextIfTooLong(String text, int charCount) {
        ...
        if (obj.length() <= charCount) {
            return text;
        }
        return "••" + StringsKt.trim((CharSequence) StringsKt.takeLast(obj, charCount)).toString();
    }

    public final int uiElementColorOnBackground(int backgroundColor) {
        return ((((double) Color.blue(backgroundColor)) * 0.114d) + ((((double) Color.green(backgroundColor)) * 0.587d) + (((double) Color.red(backgroundColor)) * 0.299d))) / 255.0d < 0.5d ? BinsView.UI_ELEMENT_ON_DARK_COLOR : BinsView.UI_ELEMENT_ON_LIGHT_COLOR;
    }
}
