...
public final class AbsoluteCutCornerShape extends CornerBasedShape {
    ...
    public Outline mo417createOutlineLjSzlW0(long size, float topStart, float topEnd, float bottomEnd, float bottomStart, LayoutDirection layoutDirection) {
        Intrinsics.checkNotNullParameter(layoutDirection, "layoutDirection");
        if (topStart + topEnd + bottomStart + bottomEnd == 0.0f) {
            return new Outline.Rectangle(SizeKt.m1115toRectuvyYCjk(size));
        }
        Path Path = AndroidPath_androidKt.Path();
        Path.moveTo(0.0f, topStart);
        Path.lineTo(topStart, 0.0f);
        Path.lineTo(Size.m1094getWidthimpl(size) - topEnd, 0.0f);
        Path.lineTo(Size.m1094getWidthimpl(size), topEnd);
        Path.lineTo(Size.m1094getWidthimpl(size), Size.m1091getHeightimpl(size) - bottomEnd);
        Path.lineTo(Size.m1094getWidthimpl(size) - bottomEnd, Size.m1091getHeightimpl(size));
        Path.lineTo(bottomStart, Size.m1091getHeightimpl(size));
        Path.lineTo(0.0f, Size.m1091getHeightimpl(size) - bottomStart);
        Path.close();
