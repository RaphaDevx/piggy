...
public final class DeviceUtils {
    ...
    public final boolean isXCoverDevice$scandit_barcode_capture() {
        List listOf = CollectionsKt.listOf((Object[]) new String[]{"(sm-g388|sm-g389).*", "(sm-g390|sm-g398).*", "(sm-g525).*", "(sm-g715).*", "(sm-g736).*"});
        if (!(listOf instanceof Collection) || !listOf.isEmpty()) {
            Iterator it = listOf.iterator();
            while (it.hasNext()) {
                Regex regex = new Regex((String) it.next(), RegexOption.IGNORE_CASE);
                INSTANCE.getClass();
                ...
                if (regex.matches(StringsKt.trim((CharSequence) StringsKt.replace$default(lowerCase, "samsung-", "", false, 4, (Object) null)).toString())) {
                    return true;
                }
            }
        }
