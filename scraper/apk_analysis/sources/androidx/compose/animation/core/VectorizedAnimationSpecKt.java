...
public final class VectorizedAnimationSpecKt {
    ...
    public static final <V extends AnimationVector> Animations createSpringAnimations(final V v, final float f, final float f2) {
        if (v != null) {
            return new Animations(f, f2) { // from class: androidx.compose.animation.core.VectorizedAnimationSpecKt$createSpringAnimations$1
                ...
                /* JADX WARN: Incorrect types in method signature: (TV;FF)V */
                {
                    this.$dampingRatio = f;
                    this.$stiffness = f2;
                    IntRange until = RangesKt.until(0, AnimationVector.this.getSize());
                    ArrayList arrayList = new ArrayList(CollectionsKt.collectionSizeOrDefault(until, 10));
                    Iterator<Integer> it = until.iterator();
                    while (it.hasNext()) {
                        arrayList.add(new FloatSpringSpec(f, f2, AnimationVector.this.get$animation_core_release(((IntIterator) it).nextInt())));
                    }
