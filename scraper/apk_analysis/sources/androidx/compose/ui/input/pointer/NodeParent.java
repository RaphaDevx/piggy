...
public class NodeParent {
    ...
    public final void m2414recursivelyRemovePointerId0FcD4WY(long pointerId) {
        int i = 0;
        while (i < this.children.getSize()) {
            Node node = this.children.getContent()[i];
            node.getPointerIds().remove(PointerId.m2416boximpl(pointerId));
            if (node.getPointerIds().isEmpty()) {
                this.children.removeAt(i);
            } else {
                node.m2414recursivelyRemovePointerId0FcD4WY(pointerId);
                i++;
            }
        }
    }
